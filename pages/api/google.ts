...
const processGoogleResults = async (googleData: { items: { title: any; link: any; displayLink: any; snippet: any; pagemap: { cse_image: { src: any; }[]; }; }[]; }, tokenLimit: number, tokenCount: number) => {
    ...
    if (googleData && googleData.items) {
      googleSources = googleData.items.map((item: { title: any; link: any; displayLink: any; snippet: any; pagemap: { cse_image: { src: any; }[]; }; }) => ({
        title: item.title,
        link: item.link,
...
export { fetchGoogleSearchResults, processGoogleResults, createAnswerPromptGoogle };